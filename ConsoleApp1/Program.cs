...
namespace Figury
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var x in listaObjetosci)
                Console.WriteLine(x);


            Console.WriteLine($"Średnia długość figur = {listaDlugosci.Average()} ");
            Console.WriteLine($"Sumaryczne pole figur = {listaPol.Sum()} ");
            Console.WriteLine($"Objętość figury największej = {listaObjetosci.Max()} ");
        }
    }
}